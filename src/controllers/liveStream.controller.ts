import { Request, Response } from 'express';
import { prisma } from '../lib/prisma';

// ==========================================
// PUBLIC ENDPOINTS
// ==========================================

export const getLiveStreamConfig = async (req: Request, res: Response) => {
  try {
    const streamConfig = await prisma.liveStream.findFirst();

    if (!streamConfig) {
      return res.status(404).json({ message: 'Live stream configuration not found' });
    }

    const { id, createdAt, ...dataToReturn } = streamConfig as any;

    res.json({
      data: dataToReturn
    });
  } catch (error) {
    console.error('Error fetching live stream config:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
};

// ==========================================
// ADMIN ENDPOINTS
// ==========================================

export const upsertLiveStream = async (req: Request, res: Response) => {
  try {
    const { link, onGoing } = req.body;
    
    // Since this is a singleton, we update the first record if it exists, or create one.
    const existingConfig = await prisma.liveStream.findFirst();

    let streamConfig;
    if (existingConfig) {
      streamConfig = await prisma.liveStream.update({
        where: { id: existingConfig.id },
        data: {
          link: link !== undefined ? link : existingConfig.link,
          onGoing: onGoing !== undefined ? onGoing : existingConfig.onGoing
        }
      });
    } else {
      streamConfig = await prisma.liveStream.create({
        data: { link, onGoing: onGoing || false }
      });
    }

    res.json({ message: 'Live stream config updated successfully', data: streamConfig });
  } catch (error) {
    console.error('Error upserting live stream config:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
};
